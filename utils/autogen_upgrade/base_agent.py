...
from utils.autogen_upgrade.autogen_fix_execution import filter_duplicate_commands
...
class ExtendedAssistantAgent(AssistantAgent, BasicConversableAgent):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)


class ExtendedUserProxyAgent(UserProxyAgent, BasicConversableAgent):
    ...
    def set_env(self,):
        if self.remote_repo_path and self.remote_repo_path not in sys.path:
            ...
            if 'PYTHONPATH' not in os.environ:
                os.environ['PYTHONPATH'] = f"{os.getcwd()}:{self.remote_repo_path}"
            else:
        ...
        if self.local_repo_path and self.local_repo_path not in sys.path:
            ...
            if 'PYTHONPATH' not in os.environ:
                os.environ['PYTHONPATH'] = f"{os.getcwd()}:{self.local_repo_path}"
            else:
    ...
    def _generate_code_execution_reply_using_executor(
        ...
        return is_exec_success, f"{code_exit}\nCode output: {code_result}"