...
from utils.special import filter_important_files
...
class RepoMap:
    TAGS_CACHE_DIR = f".cache.v{CACHE_VERSION}"
    warned_files = set()
    ...
    def token_count(self, text):
        ...
        sample_tokens = token_count(sample_text)
...
def get_scm_fname(lang):
    ...
    scm_dir = Path(__file__).parent / "queries"
    if USING_TSL_PACK:
        subdir = "tree-sitter-language-pack"
        try:
            path = scm_dir.joinpath(
                subdir,
                f"{lang}-tags.scm",
            )
            if path.exists():
...
def token_count(text: str) -> int:
    """Count tokens using GPT-4's tokenizer"""
